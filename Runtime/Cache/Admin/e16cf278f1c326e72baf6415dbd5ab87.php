...
<div class="tab-wrap">
	...
	<form id="form" action="<?php echo U('update');?>" method="post" class="form-horizontal">
                        ...
                        <?php switch($field["type"]): case "num": ?><input type="text" class="text input-mid" name="<?php echo ($field["name"]); ?>" value="<?php echo ($field["value"]); ?>"><?php break;?>
								...
								function uploadPicture<?php echo ($field["name"]); ?>(file, data){
							        ...
							        if(data.status){
							        	...
							        	$("#cover_id_<?php echo ($field["name"]); ?>").parent().find('.upload-img-box').html(
							        		'<div class="upload-pre-item ert"><img src="' + src + '"/></div><a href="javascript:;" onclick="clearImge_<?php echo ($field["name"]); ?>(this)">清除图片</a>'
							        	);
							        } else {
                           ...
                           <!--多文件上传-->
                            <?php case "file": ?><div class="controls">
									...
									<input type="hidden" name="<?php echo ($field["name"]); ?>" value="" id="file_id_<?php echo ($field["name"]); ?>"/>
									<div class="upload-img-box">
								...
								function uploadFile<?php echo ($field["name"]); ?>(file, data){
							        ...
							        if(data.status){
							        	...
							        	$("input[name="+name+"]").parent().append('<input type="hidden" name="<?php echo ($field["name"]); ?>[]" value="'+data.id+'" id="file_id_<?php echo ($field["name"]); ?>"/>');
								...
								function clearfile_<?php echo ($field["name"]); ?>(obj){
					                $(obj).prev().hide()
					                $(obj).hide();
					                $("#file_id_<?php echo ($field["name"]); ?>").val('');
					            }
								</script><?php break;?>
                            <!--多图上传-->
                            <?php case "duotu": ?><div class="page-container" >
                            ...
                            <!--多图结束-->
                            <!--图片裁剪-->
                            <?php case "img_caijian": ?><div class="yc-upload-wrap">
													        	...
													        	<div class="row">
															        <div class="col-md-12 col-sm-12 col-xs-12" style="padding-right:0;padding-left:36px;">
															        	<div style="min-height:1px;line-height:160px;text-align:center;position:relative;" ontouchstart="">
																        	...
																        	<div id="view_<?php echo ($field['name']); ?>" style="width:100px;height:90px; margin-left: -35px; display: none;"></div>
															        	</div>
														        	</div>
													        	</div>
												        	</div>
	   ...
	   //清除图片
	    function clearImges_<?php echo ($field["name"]); ?>()
			{
				$("#view_<?php echo ($field['name']); ?>").remove();
				$("#clear_id_<?php echo ($field['name']); ?>").remove();
		...
		var clipArea = new bjj.PhotoClip("#clipArea_<?php echo ($field['name']); ?>", {
			...
			clipFinish: function(dataURL) {
				...
				$('#view_<?php echo ($field['name']); ?>').css('background-size','100% 100%');
				...
				$.ajax({
					...
					success:function(msg)
					{
						...
						if(1<=msg&&!isNaN(msg))
						{
							$(".yc-upload-wrap").append('<input type="hidden" name="<?php echo ($field["name"]); ?>" value="'+msg+'" id="clear_id_<?php echo ($field['name']); ?>"/>');
							$("#view_<?php echo ($field['name']); ?>").append('<a href="javascript:;" onclick="clearImges_<?php echo ($field["name"]); ?>(this)">清除图片</a>');
						}
