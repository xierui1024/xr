<?php if (!defined('THINK_PATH')) exit(); $banner=M('document_classic_adv as a') ->join('modao_picture as b on a.img=b.id') ->order('a.id desc') ->field('a.*,b.path') ->limit(1) ->find(); $type=M('attribute')->where('id=154')->find(); $type=parse_field_attr($type['extra']); ?>
<!DOCTYPE html>
<html lang="zh">
	<head>
		<meta charset="utf-8">
			...
			<div class="ui-fullpart pt" style="padding-top: 35px;">
				<div class="container">
					<div class="ui-typelist">
						<ul role="tablist">
							<?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($i == $id): ?><li role="presentation" class="active"><?php else: ?><li role="presentation" class=""><?php endif; ?>
